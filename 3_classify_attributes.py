...
def load_ontology_classification_model(model_dir: str = None, device: torch.device = None):
    ...
    # -- model weights --
    checkpoint_path = os.path.join(model_dir, "annotated_attributes_data/attributes_classification_model.pt")
    ...
    # -- label mapping --
    mapping_path = os.path.join(model_dir, "annotated_attributes_data/training_label_mapping.json")
    with open(mapping_path, "r", encoding="utf-8") as f:
        label2id = json.load(f)
    id2label = {v: k for k, v in label2id.items()}

    # ---- load mappings ----
    with open(os.path.join(model_dir, "annotated_attributes_data/syntactic_role_mapping.json"), "r", encoding="utf-8") as f:
